#!/usr/bin/env python3
"""
Emily v25 — Narrative Hook (clean cuts)
Built on the Zamiyah Client Edit XML generator architecture.
All word edges cleaned — no trailing junk, complete thoughts.
"""
import json, os
...
# ============================================================
EDIT_ANCHORS = [
    # ── v23: NEW ASSEMBLY — energy builds, no flat spots ──
    #
    # OPEN: The vibe
    # 1. W: Wide — both, the set, the laugh
    (134.7, "We've", 136.7, "scenes", "Open Vibe", "W"),
    # 2. H: Lilliani introduces Emily by name
    (143.0, "Her", 147.0, "exciting", "Meet Emily", "H"),
    #
    # FUN: She's creative — show don't tell
    # 3. G: Quick tease — end at "mind." not "And"
    (319.3, "have", 320.66, "mind", "Creative Mind", "G"),
    # 4. H: Lilliani pulls
    (338.6, "can", 341.2, "character", "Can I See", "H"),
    # 5. G: Punchline
    (394.6, "That", 396.5, "acid", "Sulfuric Acid", "G"),
    # 6. W: Shared energy — all 3 "keep going"s
    (390.6, "Keep", 392.8, "going", "Keep Going", "W"),
    #
    # BRIDGE: Fun to real (keep the pace up)
    # 7. G: Quick personality flash
    (644.7, "I'm", 645.9, "social", "Super Social", "G"),
    # 8. G: Mom doubted her — funny AND leads to nursing (NEW)
    (664.1, "you're", 666.0, "minute", "Mom No Temper", "G"),
    # 9. G: "I carved my own path" — confident, short (NEW)
    (670.0, "definitely", 672.8, "nursing", "Carved Path", "G"),
    #
    # HEART: Lilliani asks a REAL question → Emily answers
    # 10. H: "What gives you the good feelings?" — thoughtful, specific (REPLACED)
    (688.9, "what", 693.4, "moments", "Good Feelings", "H"),
    # 11. G: Her answer — the emotional core
    (694.3, "Knowing", 698.4, "difference", "Touch Care", "G"),
    #
    # COMEDY: Lighten before the build
    # 12. G: Funny family line — end at "dinner" clean
    (857.0, "got", 860.1, "dinner", "Whats For Dinner", "G"),
    #
    # MISSION: Her vision (positive, forward)
    # 13. G: End at "2026" not trailing "and"
    (584.4, "and", 588.0, "2026", "Magic 2026", "G"),
    #
    # POWER: Set up the Tootie payoff
    # 14. G: End at "are." not trailing "What"
    (2525.2, "You", 2529.8, "are", "Know Who You Are", "G"),
    #
    # TOOTIE PAYOFF
    # 15. H: Build — she wants more
    (2422.6, "I", 2423.5, "this", "Wanna Hear", "H"),
    # 16. G: Tootie — "That hair, fabulous!"
    (2474.7, "That", 2477.2, "fabulous", "Fabulous", "G"),
    # 17. H: "Oh my gosh, I got her on my show!"
...
def resolve_anchors(anchors, words):
    ...
    for first_t, first_w, last_t, last_w, label, cam in anchors:
        ...
        # Tight padding — prevent word bleed
        # Start: 0.15s before first word (just enough for onset)
        # End: 0.20s after last word (let the thought land, no next-word bleed)
        START_PAD = 0.15
        END_PAD = 0.20
        ...
        if next_w and (next_w["start"] - lw["end"]) < 2.0:
            ...
            out_pt = lw["end"] + min(gap * 0.3, END_PAD)  # only 30% of gap, max 0.20s
        else:
...
L.append(' <sequence>')
L.append('  <name>Emily v25 Hook</name>')
...
out_comp = f"{BASE}/Emily_v25_Hook.xml"
