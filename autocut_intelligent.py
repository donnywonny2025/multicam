...
Applies transcript-aware editorial rules and exports a 3-track stack
where all tracks are cut simultaneously and the non-active angles are disabled.
"""
...
# ============================================================
# STEP 4: AI INTELLIGENCE LAYER (TRANSCRIPT DATA FUSION)
# ============================================================
print("Analyzing transcript for intelligent cuts...")
with open(TRANSCRIPT_PATH, "r") as f:
    ts_data = json.load(f)

# Build a list of questions & laughs for context
questions = []
laughs = []
for t_seg in ts_data.get('segments', []):
    ...
    if '[laugh' in txt.lower() or 'haha' in txt.lower():
...
for i, (spk, s, e) in enumerate(pass2):
    ...
    # Check if a question was asked right before this segment
    is_answering_question = False
    if i > 0 and pass2[i-1][0] != spk and spk in ('H','G'):
        prev_s, prev_e = pass2[i-1][1], pass2[i-1][2]
        # Was there a question in the last 3-4 seconds of the previous speaker?
        for (q_s, q_e) in questions:
            if prev_e - 4.0 <= q_e <= prev_e + 1.0:
                is_answering_question = True
                break
    
    # 1. ANTICIPATORY CUT: If they are answering a question, cut to them 0.6s early
    if is_answering_question and dur > 2.0 and pass3_ai:
        early_cut = max(pass3_ai[-1][1], s - 0.6)
        pass3_ai[-1] = (pass3_ai[-1][0], pass3_ai[-1][1], early_cut) # Shorten prev
        s = early_cut # Lengthen current
        #print(f"AI decision: Anticipatory cut at {s:.1f}s because of a question.")
        
    # 2. STORY PRESERVATION: If interjection is very short, and they are in the middle of a story, ignore it.
    if dur < MIN_INTERJECTION:
        # It's an interjection. 
        if pass3_ai:
            # Revert to previous speaker
...
print("Generating v3 Intelligent 3-Track XML...")
...
L.append('  <sequence>')
L.append('    <name>Emily Intelligent Cut v3 - 3 Track</name>')
...
# V1 = W (Wide)
# V2 = H (Lillani)
# V3 = G (Emily / Wide fallbacks if WIDE fails, but WIDE is everywhere)

spk_to_track = {'W': 'V1', 'S': 'V1', 'H': 'V2', 'G': 'V3'}

for trk_idx, (trk_id, spk_flag, _) in enumerate(track_configs): # Loop all 3 tracks
    L.append('        <track>')
    L.append('          <enabled>TRUE</enabled>')
    L.append('          <locked>FALSE</locked>')
    for (active_spk, s, e) in merged:
        # For every segment, we lay down a clip for THIS track (trk_id)
        # Exception: if camera wasn't rolling
        fid, src_s = find_src(trk_id, s)
        if fid is None:
            continue
            
        name, url, dur_f = file_entries[fid]
        tl_s_f = sec_to_frames(s)
        tl_e_f = sec_to_frames(e)
        ...
        # Is this track the active one chosen by the AI?
        is_active = (spk_to_track[active_spk] == trk_id)
        enable_val = "TRUE" if is_active else "FALSE"
        ...
        L.append(f'            <enabled>{enable_val}</enabled>')
