#!/usr/bin/env python3
"""
AI Multicam Auto-Cut v6 (Dynamic Locked)
Enforces absolute 30fps sequence timebase to prevent Premiere drift.
Strictly adheres to the Golden Layout / North Star zero-anchoring.
"""
...
MIN_INTERJECTION = 0.7
RAPID_THRESHOLD = 2.5
OPENING_WIDE_SEC = 4.0
SR = 16000
ANTICIPATORY_LEAD = 0.8

print("Extracting MIC1 (Lillani)...")
mic1 = core.extract_mono(MIC1_PATH, SR, "v6_mic1")
mic2 = core.extract_mono(MIC2_PATH, SR, "v6_mic2")
...
for t_seg in ts_data.get('segments', []):
    ...
    if any(k in txt.lower() for k in ["[laugh", "haha", "funny"]): laughs.append((seg_s, seg_e))
    if any(k in txt.lower() for k in ["emily", "silly", "you"]): emily_mentions.append((seg_s, seg_e))
...
# STEP 5: Generate XML 
# ============================================================
print("Applying 30fps Absolute Math...")
...
seq_dur_f = sec_to_frames_30(min_len / SR)
...
L.append('  <sequence>')
L.append(f'    <name>Emily Dynamic Cut v6 - Locked 30fps</name>')
L.append(f'    <duration>{seq_dur_f}</duration>')
...
for trk_idx, (trk_id, spk_flag, _) in enumerate(track_configs):
    ...
    for (active_spk, s, e) in merged:
        ...
        tl_s_f, tl_e_f = sec_to_frames_30(s), sec_to_frames_30(e)
        in_f = sec_to_frames_30(src_s)
...
mic_configs = [("MIC1.WAV", MIC1_PATH), ("MIC2.WAV", MIC2_PATH)]
for mic_idx, (m_id, m_path) in enumerate(mic_configs):
    m_dur_s, _ = core.ffprobe_metadata(m_path)
    m_dur_f = sec_to_frames_30(m_dur_s)
    url = "file://localhost" + core.quote(m_path)
    L.append('        <track>')
...
out_comp = f"{BASE}/Emily_v6_DYNAMIC_LOCKED.xml"
