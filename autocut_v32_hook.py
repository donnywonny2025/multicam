...
def resolve_anchors(anchors, words):
    ...
    for first_t, first_w, last_t, last_w, label, cam in anchors:
        ...
        # Absolute precision override mode: if word string is None, cut EXACTLY at float time
        if first_w is None:
            in_pt = first_t
        else:
            fw = None
...
print("\n1. Resolving Word Anchors (HIGH PRECISION)...")
...
L.append(' <sequence>')
L.append('  <name>Emily v32 Precision</name>')
...
out_comp = f"{BASE}/Emily_v32_Hook.xml"
